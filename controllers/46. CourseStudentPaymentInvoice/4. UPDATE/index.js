...
const STUDENT_UPDATE_PAYMENT_INVOICE = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        /* ── 2. Invoice татах ── */
        const invoice = await prismaService.course_student_payment_invoice.findUnique({
            where: { id: parseInt(id) },
            include: {
                course_student_payment: {
                    include: {
                        course_student: true,
        ...
        /* ── 3. Төлбөр аль хэдийн төлөгдсөн эсэх шалгах ── */
        ...
        if (bylStatus === "void" || bylStatus === "cancelled") {
            await prismaService.course_student_payment_invoice.update({
                where: { id: parseInt(id) },
                data: { status: "void" }
            });
            return res.status(200).json({
        ...
        if (bylStatus === "paid") {
            ...
            // Invoice шинэчлэх
            const updatedInvoice = await prismaService.course_student_payment_invoice.update({
                where: { id: parseInt(id) },
                data: {
                    status: "paid",
            ...
            // Student payment шинэчлэх
            if (invoice.student_payment) {
                await prismaService.course_student_payment.update({
                    where: { id: invoice.student_payment },
                    data: {
                        status: "Төлөгдсөн",
            ...
            // Бүтэн мэдээллийг дахин татаж буцаах
            const fullInvoice = await prismaService.course_student_payment_invoice.findUnique({
                where: { id: parseInt(id) },
                include: {
                    course_student_payment: {
                        include: {
            ...
            return res.status(200).json({
                success: true,
                data: fullInvoice,
                message: "Төлбөр амжилттай баталгаажлаа. 14 хоногийн эрх нэмэгдлээ."
            });
        }
...
module.exports = STUDENT_UPDATE_PAYMENT_INVOICE;